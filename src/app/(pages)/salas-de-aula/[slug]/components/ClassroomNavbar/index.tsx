    ...
    >
      <ul className="mx-auto flex w-full max-w-3xl items-center justify-between">
        {CLASSROOM_NAVBAR_LINKS.map((link: string, index: number) => (
          <li
            className="w-full cursor-pointer border-b-2 border-transparent pb-2 text-center text-sm transition-all duration-300 first:border-indigo-500 first:font-semibold first:text-indigo-500 hover:text-indigo-500"
            key={`${link}-${index}`}
          >
            {link}
          </li>
